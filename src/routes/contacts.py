...
async def get_birthday_contacts(birthday_days: int, db: AsyncSession = Depends(get_db)):
    contacts = await repository_contacts.get_birthday_contacts(birthday_days, db)
    if contacts is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Contact not found...')
    return contacts
...
async def get_contacts_by_params(name: str | None = None, surname: str | None = None, email: str | None = None, db: AsyncSession = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_by_params(name, surname, email, db)
    if contacts is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Contact not found...')
    return contacts
